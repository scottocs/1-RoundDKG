...
def batchDecryption():    
    ...
    global gp
    pks = {} 
    user_keys ={'GID': "bob", 'keys': {}}
    print("n=",n)
    for i in range(0, n):
        (pk, sk) = authKeys[i]
        user_attributesi = ['ATTR@AU'+str(i)] 
        user_keysi = maabe.multiple_attributes_keygen(gp, sk, "bob", user_attributesi)         
        # user_keys = {'GID': "bob", 'keys': merge_dicts(user_keys["keys"], user_keysi)}        
        user_keys['keys'].update(user_keysi)
        print(user_keys)
        pks["AU"+str(i)]=pk
    t=int(n/2)+1
    # t=int(n/2)-1
    ...
    for i in range(0, n):
        ...
        M = (gp['g1'], multiply(gp['g2'], maabe.random()))#gp["egg"]**maabe.random()\        
        ct = maabe.encrypt(gp, pks, M, acp)  
        CTList.append(ct)
        MsgList.append(M)
    ts=time.time()
    for i in range(0, len(CTList)):
        decryptedMs = maabe.decrypt(gp, user_keys, CTList[i]) 
        oriMs = pairing(MsgList[i][0],MsgList[i][1])
        if decryptedMs != oriMs:
            print("error decryption",oriMs,decryptedMs)
            exit()
        else:
            print(i,"correct decryption")
    

    print("n=",n, "overall time decryption one by one:",time.time()-ts)
    ts = time.time()
    CTAgg = CTList[0]
    MsgAgg = list(MsgList[0])
    for i in range(1, len(CTList)):
        CTAgg = maabe.aggregateCT(gp, CTAgg, CTList[i])
        MsgAgg[1] = add(MsgAgg[1], MsgList[i][1])
    
    decryptedMsg = maabe.decrypt(gp, user_keys, CTAgg) 
    print("n=",n,decryptedMsg == pairing(MsgAgg[0],MsgAgg[1]), "batchDecryption time:",time.time()-ts)
